...
plot.ts(returns, type = "l", main = "ICBP Stock Returns")
...
library(TSA)
eacf(returns)
# Calon-calon model yang didapat (0,0,0),(1,0,1),(2,0,2) dan (3,0,3)
# namun setelah diuji coba, hanya model (0,0,0) dan (1,0,1) yang berhasil
# korvergen saat proses running. Sementara model lainnya mengalami
# divergen di window tertentu.
Arima_model <- list(c(0,0,0),c(1,0,1))
...
for (i in window_size:length(returns)) {
  ...
  for (j in 1:length(Arima_model)){
    # Fit ARIMA model on training data
    model_arima <- arima(train_data,order = Arima_model[[j]])
    ...
    model_garch <- ugarchspec(mean.model = list(armaOrder = Arima_model[[j]]),
